...
\begin{tcolorbox}[height=3cm, valign=center]\centering
	\textcolor{loestc}{Eine ganzrationale Funktion \(f(x)\) vom Grad \(n\) hat \textbf{maximal} \(n\) Nullstellen.}
\end{tcolorbox}
...
Dieses Verfahren kann nur dann angewendet werden, wenn jeder Summand über mindestens ein \(x\) verfügt. Wir klammern so viele \(x\) wie möglich vor ohne, dass negative Hochzahlen entstehen (kleinste Hochzahl) und wenden dann den Satz vom Nullprodukt an.
...
Der Begriff Substitution kommt aus dem Lateinischen und bedeutet Ersetzen oder Austauschen. Dieses Verfahren kann nur dann angewendet werden, wenn die Gleichung von der Form\newline
\(ax^{2n}+bx^n+c=0\) ist, d.h. es müssen drei Summanden vorhanden sein, einer ohne \(x\) und bei den beiden anderen muss die eine Hochzahl doppelt so groß sein wie die andere Hochzahl.
